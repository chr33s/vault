...
Every tier is a `BlobCipher` (`cli/blobcipher.ts` — one shared shape, no
hand-mirrored copies) over `makeBlobKeyStore`'s on-disk wrapped blob, so the
disk plumbing, id validation, and provider resolution are shared; the spawn
transports share one `spawnCollect` helper (`cli/spawn.ts`).
...
- [x] **Windows Hello strong tier (`KeyCredentialManager`)** — spec §3.5. Shipped as `windows-hello` (`cli/hello.ts` + `native/hello-helper`): wraps the DUK under `HKDF(RequestSignAsync(challenge), salt=challenge)` → AES-256-GCM, per-blob random challenge, keystore id bound as AAD; **enrollment self-tests signature determinism** (signs twice, refuses the tier on mismatch — the CNG/NCrypt fallback stays documented-but-unbuilt since KeyCredential signs RSA-2048/PKCS#1 v1.5). Sequenced as planned: (1) the off-Windows-testable `BlobCipher` + spawn transport, unit-tested against a fake-sign oracle and a stub helper (`test/hello.test.ts`); (2) the C# (CsWinRT) helper speaking a base64 stdin/stdout `available/sign [--create]` protocol (blobs stay CLI-side — the helper only signs, so the DUK never touches it; the unlock path never mints a credential), with the PowerShell WinRT-projection fallback opt-in via `$VAULT_HELLO_PS=1` (dev only — unsigned); (3) caller-auth via **Authenticode** (`WinVerifyTrust` + publisher-thumbprint pinning; `VAULT_HELLO_STRICT` fails closed when unsigned). Discovery: `$VAULT_HELLO_HELPER` or a sibling `vault-hello-helper.exe`; precedes `windows-dpapi` when present. CI compiles the helper and probes `available` on a Windows runner; the gesture paths still need verification on a real Hello-enrolled host (same status the TBS transport ships with).
